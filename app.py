...
if uploaded_file:
    ...
    if df is not None:
        ...
        k2.metric("主要風險", f_df["事件類別"].mode()[0] if not f_df.empty else "-")
        k3.metric("本期佔比", f"{round(len(f_df)/len(df)*100, 1)}%")
        ...
        with tab_total:
            ...
            with col_r:
                ...
                dept_rank = f_df["發生單位"].value_counts().reset_index().head(10)
    ...
    # 未上傳時的導引畫面
    st.info("請上傳 Excel 檔案以啟用儀表板。系統將自動合併 111-114 年數據並清理格式。")
